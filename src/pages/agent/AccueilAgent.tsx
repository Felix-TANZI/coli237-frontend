...
export function AccueilAgent() {
  ...
  const {
    enAttente: aSynchroniser,
    enCours: synchroEnCours,
    lancerSynchro,
  } = useSynchro();
      ...
      <div className="max-w-md md:max-w-5xl mx-auto md:px-6 md:py-6">
        ...
        >
            ...
            <div>
              ...
              {aSynchroniser > 0 ? (
                ...
                >
                  ...
                  {enLigne && !synchroEnCours && (
                    <i className="ti ti-refresh text-xs ml-0.5" />
                  )}
                </button>
              ) : enLigne ? (
                <div
                  className="flex items-center gap-1.5 mt-2 w-fit text-xs rounded-lg px-2.5 py-1"
                  style={{
                    background: "rgba(31,184,158,.15)",
                    border: "1px solid rgba(31,184,158,.3)",
                    color: "#5DCAA5",
                  }}
                >
          ...
          <div className="hidden md:flex gap-6 relative z-10">
            <StatHero
              valeur={data?.recenses ?? 0}
              label={t("agent.recenses")}
              couleur="#5DCAA5"
            />
            <StatHero
              valeur={data?.attente ?? 0}
              label={t("agent.enAttente")}
              couleur="#F5B87A"
            />
            <StatHero
              valeur={data?.valides ?? 0}
              label={t("agent.valides")}
              couleur="#fff"
            />
          </div>
        ...
        <div className="px-4 md:px-0">
          ...
          <div className="grid grid-cols-2 md:grid-cols-[1fr_1fr_320px] gap-3 mt-4">
            <button
              ...
              style={{
                background: "linear-gradient(135deg,#1FB89E,#17A2B8)",
                boxShadow: "0 8px 22px rgba(31,184,158,.3)",
              }}
            >
              ...
              <div>
                <div className="font-semibold text-sm md:text-base">
                  {t("agent.recenserCoursier")}
                </div>
                <div className="text-[11px] text-white/75 mt-0.5">
                  {t("agent.coursierFreelance")}
                </div>
              </div>
            </button>
            ...
            <button
              ...
              className="ag-anim ag-d2 rounded-2xl p-4 md:p-5 text-white text-left flex flex-col justify-between min-h-[120px] hover:-translate-y-0.5 transition-transform"
              style={{
                background: "linear-gradient(135deg,#F28C28,#e07d1c)",
                boxShadow: "0 8px 22px rgba(242,140,40,.3)",
              }}
            >
              <div className="w-11 h-11 rounded-xl bg-white/20 flex items-center justify-center text-2xl">
              ...
              <div>
                <div className="font-semibold text-sm md:text-base">
                  {t("agent.recenserEntreprise")}
                </div>
                <div className="text-[11px] text-white/80 mt-0.5">
                  {t("agent.partenaireLivraison")}
                </div>
              </div>
            </button>
            ...
            <div
              className="ag-anim ag-d3 col-span-2 md:col-span-1 bg-white rounded-2xl border border-gray-200/70 p-4"
              style={{ boxShadow: OMBRE }}
            >
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-coli-encre">
                  {t("agent.monRythme")}
                </span>
              ...
              >
                {data?.recents.map((r) => {
                  ...
                  return (
                    <div
                      key={r.id}
                      className="flex items-center gap-3 px-4 py-3 border-b border-gray-50 last:border-none hover:bg-gray-50/60 transition"
                    >
                      <div
...
function StatCarte({
  ...
  return (
    <div
      className="ag-anim ag-d2 bg-white rounded-xl p-2.5 border border-gray-200/70"
      style={{ boxShadow: OMBRE }}
    >
      <div
