      ...
      <main className="px-4 sm:px-6 py-5 sm:py-6 max-w-7xl mx-auto">
        ...
        <div className="hidden md:block bg-white rounded-2xl border border-gray-100 overflow-hidden">
          ...
          {!isLoading && filtres.length === 0 && (
            <p className="text-sm text-gray-400 py-8 text-center">{t('coursiers.aucun')}</p>
          )}
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center text-xs font-semibold text-white shrink-0" style={{ background: couleurAvatar(c.nom) }}>
                    {initiales(c.nom)}
                  </div>
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-coli-encre truncate">{c.nom}</div>
                    <div className="text-xs text-gray-400 truncate">{c.telephone}</div>
                  </div>
                </div>
                <span className="text-sm text-gray-600 flex items-center gap-1.5">
                  <i className={`ti ${ICONE_VEHICULE[c.typeVehicule] ?? 'ti-dots'} text-coli-vert`} />
                  {c.typeVehicule}
                </span>
                ...
                <div className="flex gap-1.5 justify-end">
                  {c.statut === 'EN_ATTENTE' && (
                    <>
                      <button onClick={() => valider.mutate(c.id)} disabled={valider.isPending} aria-label={t('coursiers.valider')} className="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-coli-vert hover:text-white flex items-center justify-center transition">
                        <i className="ti ti-check" />
                      </button>
                      <button onClick={() => rejeter.mutate(c.id)} disabled={rejeter.isPending} aria-label={t('coursiers.rejeter')} className="w-8 h-8 rounded-lg bg-red-50 text-red-700 hover:bg-red-500 hover:text-white flex items-center justify-center transition">
                        <i className="ti ti-x" />
                      </button>
                    </>
                  )}
                  <button onClick={() => setSelection(c)} aria-label={t('coursiers.details')} className="w-8 h-8 rounded-lg bg-gray-100 text-gray-600 hover:bg-coli-encre hover:text-white flex items-center justify-center transition">
        ...
        <div className="md:hidden space-y-2.5">
          {filtres.map((c) => {
            const s = STATUT[c.statut] ?? STATUT.EN_ATTENTE;
            return (
              <div key={c.id} className="bg-white rounded-xl border border-gray-100 p-3.5" onClick={() => setSelection(c)}>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center text-xs font-semibold text-white shrink-0" style={{ background: couleurAvatar(c.nom) }}>
                    {initiales(c.nom)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-coli-encre truncate">{c.nom}</div>
                    <div className="text-xs text-gray-400">{c.typeVehicule} · {c.ville ?? '—'}</div>
                  </div>
                ...
                {c.statut === 'EN_ATTENTE' && (
                  <div className="flex gap-2 mt-3" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => valider.mutate(c.id)} className="flex-1 py-2 rounded-lg bg-coli-vert text-white text-xs font-semibold flex items-center justify-center gap-1">
                      <i className="ti ti-check" />{t('coursiers.valider')}
                    </button>
                    <button onClick={() => rejeter.mutate(c.id)} className="flex-1 py-2 rounded-lg border border-red-200 text-red-700 text-xs font-semibold flex items-center justify-center gap-1">
                      <i className="ti ti-x" />{t('coursiers.rejeter')}
                    </button>
                  </div>
                )}
              </div>
