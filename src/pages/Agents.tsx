...
export function Agents() {
  ...
  const [mdpAffiche, setMdpAffiche] = useState<{
    mdp: string;
    reinit: boolean;
  } | null>(null);
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="bg-coli-orange text-white rounded-xl px-3.5 py-2.5 text-sm font-semibold flex items-center gap-1.5 hover:bg-coli-orange-fonce transition shrink-0"
            >
    ...
    <div className="grid grid-cols-[2fr_2fr_1fr_1fr_1.5fr] px-5 py-3 items-center border-b border-gray-50 last:border-none hover:bg-gray-50/50 transition">
      <div className="flex items-center gap-3">
        <div
          ...
          style={{ background: couleurAvatar(agent.nom) }}
        >
          {initiales(agent.nom)}
        </div>
      ...
      <div className="flex gap-1.5 justify-end">
        <button
          onClick={onReinit}
          aria-label={t("agents.reinitialiser")}
          title={t("agents.reinitialiser")}
          className="w-8 h-8 rounded-lg bg-gray-100 text-gray-600 hover:bg-coli-cyan hover:text-white flex items-center justify-center transition"
        >
          <i className="ti ti-key" />
        </button>
        <button
          onClick={onBasculer}
          aria-label={estActif ? t("agents.suspendre") : t("agents.reactiver")}
          title={estActif ? t("agents.suspendre") : t("agents.reactiver")}
          className="w-8 h-8 rounded-lg bg-gray-100 text-gray-600 hover:bg-coli-orange hover:text-white flex items-center justify-center transition"
        >
          <i
            className={`ti ${estActif ? "ti-player-pause" : "ti-player-play"}`}
          />
        </button>
        <button
          onClick={onArchiver}
          aria-label={t("agents.archiver")}
          title={t("agents.archiver")}
          className="w-8 h-8 rounded-lg bg-gray-100 text-gray-600 hover:bg-red-500 hover:text-white flex items-center justify-center transition"
        >
