...
function initiales(nom: string): string {
  ...
  return m.slice(0, 2).map((x) => x[0]).join('').toUpperCase();
}
...
function regionDeVille(ville: string): string | null {
  ...
  const carte: Record<string, string> = {
    yaound: 'Centre', douala: 'Littoral', bafoussam: 'Ouest',
    bamenda: 'Nord-Ouest', buea: 'Sud-Ouest', garoua: 'Nord',
    maroua: 'Extrême-Nord', ngaound: 'Adamaoua', bertoua: 'Est',
    ebolowa: 'Sud', kribi: 'Sud', limbe: 'Sud-Ouest', edea: 'Littoral',
  };
...
export function PanneauContexte({
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(160deg,#0E1A24,#16334a)', boxShadow: '0 8px 24px rgba(14,26,36,.18)' }}
    >
      {/* Aperçu de la fiche */}
      ...
      <div className="relative z-10 flex items-center gap-3 bg-white/[.06] border border-white/10 rounded-xl p-3.5 mb-4">
        <div className="w-11 h-11 rounded-xl flex items-center justify-center font-bold text-base shrink-0" style={{ background: couleurAvatar }}>
          {initiales(nom || '?')}
        </div>
      ...
      <svg viewBox={CARTE_VIEWBOX} className="w-full h-auto max-h-52 relative z-10" aria-hidden="true">
        ...
        {regionActive &&
          REGIONS_CAMEROUN.filter((r) => r.nom === regionActive).map((r) => (
            <g key="pointeur">
              <circle className="cm-ping" cx={r.cx} cy={r.cy} style={{ stroke: '#F28C28' }} />
      ...
      <div className="absolute -right-8 -bottom-8 w-32 h-32 rounded-full opacity-15" style={{ background: 'radial-gradient(circle,#1FB89E,transparent 70%)' }} />
    </div>
  );
}