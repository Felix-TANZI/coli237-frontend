    ...
    <div className="min-h-screen bg-coli-craie">
      <div className="max-w-6xl mx-auto px-4 py-5 md:px-6 md:py-7">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <div
              className="font-extrabold text-lg md:text-xl text-coli-encre"
              style={{ fontFamily: "Sora, Inter" }}
            >
              {titre}
            </div>
            <div className="text-[11px] md:text-xs text-gray-400">
              {t("recensement.etapeSur", { n: etapeCourante + 1, total })} ·{" "}
              ...
              >
                <div className="flex flex-col items-center gap-1.5">
                  <div
                    ...
                    style={
                      courant
                        ? { boxShadow: "0 0 0 4px rgba(14,26,36,.1)" }
                        : undefined
                    }
                  >
                  ...
                  <span
                    className={`text-[11px] font-medium whitespace-nowrap ${courant ? "text-coli-encre" : fait ? "text-gray-500" : "text-gray-300"}`}
                  >
                    {nomEtape}
                  </span>
                </div>
                {i < total - 1 && (
                  <div
                    ...
                    style={{ background: fait ? "#1FB89E" : "#e2e8ec" }}
                  />
                )}
              </div>
            );
        ...
        <div
          className={`grid gap-5 ${panneauContexte ? "md:grid-cols-[1fr_340px]" : "max-w-2xl mx-auto"}`}
        >
          {/* Carte formulaire */}
          <div
            key={etapeCourante}
            ...
            style={{
              boxShadow:
                "0 1px 3px rgba(14,26,36,.04), 0 8px 24px rgba(14,26,36,.06)",
            }}
          >
            {children}
          </div>
          ...
          {panneauContexte && (
            <div className="order-first md:order-none">{panneauContexte}</div>
          )}
        </div>

        {/* Boutons navigation */}
        <div
          className={`flex justify-between mt-6 ${panneauContexte ? "md:max-w-[calc(100%-360px)]" : "max-w-2xl mx-auto"}`}
        >
          {etapeCourante > 0 ? (
            <button
              onClick={onPrecedent}
