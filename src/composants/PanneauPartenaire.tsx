    ...
    <>
      ...
      <aside className="fixed top-0 right-0 bottom-0 w-full sm:w-[360px] bg-white shadow-2xl z-50 p-6 overflow-y-auto">
        ...
        <div className="border-t border-gray-100 pt-3.5 mb-3.5 space-y-1">
          <Ligne
            label={t("partenaires.responsable")}
            valeur={`${partenaire.responsableNom} · ${partenaire.responsableTelephone}`}
          />
          <Ligne
            label={t("partenaires.region")}
            valeur={
              [partenaire.ville, partenaire.quartier]
                .filter(Boolean)
                .join(" · ") || "—"
            }
          />
          {partenaire.niu && (
            <Ligne label={t("partenaires.niu")} valeur={partenaire.niu} />
          )}
          {partenaire.registreCommerce && (
            <Ligne
              label={t("partenaires.registre")}
              valeur={partenaire.registreCommerce}
            />
          )}
          <Ligne
            label={t("partenaires.coursiersRattaches")}
            valeur={String(nbCoursiers)}
          />
          {partenaire.mobileMoneyNumero && (
            <Ligne
              label={t("partenaires.mobileMoney")}
              valeur={`${partenaire.mobileMoneyNumero}${partenaire.mobileMoneyOperateur ? " · " + partenaire.mobileMoneyOperateur : ""}`}
            />
          )}
        </div>
        ...
        {partenaire.documents && partenaire.documents.length > 0 && (
          <>
            ...
            {partenaire.documents.map((d) => (
              <div
                key={d.id}
                className="flex items-center gap-2.5 p-2.5 bg-gray-50 rounded-xl mb-2"
              >
                ...
                <a
                  href={`${import.meta.env.VITE_API_URL ?? "http://localhost:3000"}/${d.chemin}`}
                  target="_blank"
                  rel="noreferrer"
                  className="text-coli-cyan"
                  aria-label="Voir"
                >
