...
export function CarteDensite({ parRegion }: { parRegion: Record<string, number> }) {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4">
        ...
        {REGIONS_CAMEROUN.map((r) => {
          ...
          return (
            <path
              ...
              onClick={() =>
                setVerrouille((v) => (v === r.nom ? null : r.nom))
              }
            >
              <title>{r.nom}</title>
            </path>
      ...
      <div className="flex gap-3 mt-2 text-[10.5px] text-gray-500">
        <span className="flex items-center gap-1">
          <span className="w-2.5 h-2.5 rounded" style={{ background: '#1FB89E' }} />
          {t('tableau.elevee')}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2.5 h-2.5 rounded" style={{ background: '#9FE1CB' }} />
          {t('tableau.moyenne')}
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2.5 h-2.5 rounded" style={{ background: '#e3edf0' }} />
          {t('tableau.faible')}
        </span>
      </div>
    </div>
