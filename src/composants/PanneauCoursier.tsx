    ...
    <>
      ...
      <aside className="fixed top-0 right-0 bottom-0 w-full sm:w-[360px] bg-white shadow-2xl z-50 p-6 overflow-y-auto">
        ...
        <div className="border-t border-gray-100 pt-3.5 mb-3.5 space-y-1">
          <Ligne
            label={t("coursiers.vehicule")}
            valeur={`${coursier.typeVehicule}${coursier.plaque ? " · " + coursier.plaque : ""}`}
          />
          <Ligne
            label={t("coursiers.region")}
            valeur={
              [coursier.ville, coursier.quartier].filter(Boolean).join(" · ") ||
              "—"
            }
          />
          {coursier.aPermis && (
            <Ligne
              label={t("coursiers.permis")}
              valeur={
                coursier.permisCategorie
                  ? `Catégorie ${coursier.permisCategorie}`
                  : "Oui"
              }
            />
          )}
